...
__global__ void kernel_target( CudaPic picture )
{
	...
	//picture.m_p_uchar3[y * picture.m_size.x + x] = { 255, 255, 255 };
	...
	if(distance > 100)
		picture.at3(x, y) = { 255, 255, 0 };

	if(distance > 150)
		picture.at3(x, y) = { 0, 60, 255 };
...
__global__ void kernel_rotate(CudaPic picture, CudaPic rotated)
{
	// X,Y coordinates and check image dimensions
	int y = blockDim.y * blockIdx.y + threadIdx.y;
	int x = blockDim.x * blockIdx.x + threadIdx.x;
	if ( y >= rotated.m_size.y ) return;
	if ( x >= rotated.m_size.x ) return;

	int y2 = x;
	int x2 = rotated.m_size.y - y - 1;

	rotated.at3(y, x) = picture.at3(y2, x2);
}

void cu_rotate(CudaPic t_pic, CudaPic t_rotated)
{
	cudaError_t l_cerr;

	// Grid creation, size of grid must be equal or greater than images
	int l_block_size = 32;
	dim3 l_blocks( ( t_rotated.m_size.x + l_block_size - 1 ) / l_block_size,
				   ( t_rotated.m_size.y + l_block_size - 1 ) / l_block_size );
	dim3 l_threads( l_block_size, l_block_size );
	kernel_rotate<<< l_blocks, l_threads >>>(t_pic, t_rotated);
...
__global__ void kernel_zmensit(CudaPic picture, CudaPic smaller)
{
	// X,Y coordinates and check image dimensions
	int y = blockDim.y * blockIdx.y + threadIdx.y;
	int x = blockDim.x * blockIdx.x + threadIdx.x;
	if (y >= smaller.m_size.y) return;
	if (x >= smaller.m_size.x) return;

	int imageX = x * 2;
	int imageY = y * 2;

	uchar b_px = (picture.at3(imageX, imageY).x + picture.at3(imageX + 1, imageY).x + picture.at3(imageX, imageY + 1).x + picture.at3(imageX + 1, imageY + 1).x)/4;
	uchar g_px = (picture.at3(imageX, imageY).y + picture.at3(imageX + 1, imageY).y + picture.at3(imageX, imageY + 1).y + picture.at3(imageX + 1, imageY + 1).y)/4;
	uchar r_px = (picture.at3(imageX, imageY).z + picture.at3(imageX + 1, imageY).z + picture.at3(imageX, imageY + 1).z + picture.at3(imageX + 1, imageY + 1).z)/4;

	smaller.at3(x, y) = {b_px, g_px, r_px};
}

void cu_zmensit(CudaPic t_pic, CudaPic t_smaller)
{
	cudaError_t l_cerr;

	// Grid creation, size of grid must be equal or greater than images
	int l_block_size = 16;
	dim3 l_blocks( ( t_smaller.m_size.x + l_block_size - 1 ) / l_block_size,
				   ( t_smaller.m_size.y + l_block_size - 1 ) / l_block_size );
	dim3 l_threads( l_block_size, l_block_size );
	kernel_zmensit<<< l_blocks, l_threads >>>(t_pic, t_smaller);
