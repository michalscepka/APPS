...
int main( int t_numarg, char **t_arg )
{
	...
	cv::Mat cat = cv::imread("cat.jpg", CV_LOAD_IMAGE_COLOR);

	cv::Mat cat_rotated(cat.size().width, cat.size().height, CV_8UC3);

	// data for CUDA
	CudaPic pic_cat, pic_cat_rotated;
	pic_cat.m_size.x = cat.size().width;
	pic_cat_rotated.m_size.x = cat_rotated.size().width;

	pic_cat.m_size.y = cat.size().height;
	pic_cat_rotated.m_size.y = cat_rotated.size().height;

	pic_cat.m_p_uchar3 = (uchar3 *)cat.data;
	pic_cat_rotated.m_p_uchar3 = (uchar3 *) cat_rotated.data;

	// Function calling from .cu file
	cu_rotate(pic_cat, pic_cat_rotated);
	...
	cv::Mat cat_smaller(cat.size().height/2, cat.size().width/2, CV_8UC3);

	CudaPic pic_cat_smaller;
	pic_cat_smaller.m_size.x = cat_smaller.size().width;
	pic_cat_smaller.m_size.y = cat_smaller.size().height;
	pic_cat_smaller.m_p_uchar3 = (uchar3 *) cat_smaller.data;

	cu_zmensit(pic_cat, pic_cat_smaller);
	cv::imshow("Smaller", cat_smaller);
